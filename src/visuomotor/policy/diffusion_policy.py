...
class DiffusionPolicy2:
  ...
  def __init__(self, config, device):
    ...
    vision_feature_dim = 512 * config.obs_horizon
...
class DiffusionPolicy2Cam:
  ...
  def predict_noise(self, arm_image, depth_image, agent_pos, noised_action, t):
    arm_image_features = self.nets['vision_encoder'](arm_image.flatten(end_dim=1))
    # TODO: dangerous
    # arm_image_features = arm_image_features.reshape(*arm_image.shape[:2], -1)

    depth_image_features = self.nets['vision_encoder'](depth_image.flatten(end_dim=1))
    # TODO: dangerous
    # depth_image_features = depth_image_features.reshape(*depth_image.shape[:2], -1)

    # TODO: dangerous
    agent_pos = agent_pos.flatten(start_dim=1)

    
    obs = torch.cat([arm_image_features, depth_image_features, agent_pos], dim=-1)
  ...
  def action(self, arm_image, depth_image, agent_pos, batch_size):
    ...
    for t in self.noise_scheduler.timesteps:
      noise_pred = self.predict_noise(arm_image, depth_image, agent_pos, naction, t)
      # remove noise
      naction = self.noise_scheduler.step(
          model_output=noise_pred,
          timestep=t,
  ...
  def compute_train_loss(self, arm_image: torch.Tensor, depth_image: torch.Tensor, poses: torch.Tensor, target_actions: torch.Tensor) -> torch.Tensor:
    ...
    noise = torch.randn(target_actions.shape, device=device)
    noisy_actions = self.forward_process(target_actions, noise, t)
    noise_pred = self.predict_noise(arm_image, depth_image, poses, noisy_actions, t)
    loss = nn.functional.mse_loss(noise_pred, noise)
  ...
  @staticmethod
  def from_ema(config, device, ema):
    new_policy = DiffusionPolicy2Cam(config, device)
    ema.copy_to(new_policy.nets.parameters())
    return new_policy
